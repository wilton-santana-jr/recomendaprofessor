	...
	<body>
		...
		<div id="show-categoria" class="content scaffold-show" role="main">
			...
			<ol class="property-list categoria">
				...
				<g:if test="${categoriaInstance?.area}">
				<li class="fieldcontain">
					<span id="area-label" class="property-label"><g:message code="categoria.area.label" default="Area" /></span>
					
						<span class="property-value" aria-labelledby="area-label"><g:link controller="area" action="show" id="${categoriaInstance?.area?.id}">${categoriaInstance?.area?.encodeAsHTML()}</g:link></span>
